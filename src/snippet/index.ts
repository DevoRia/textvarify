...
export class Snippet {
  ...
  constructor(private readonly border: SnippetBorder | OwnBorder, private readonly borderLength: number = 1) {
    this.mirrorBorder = this.setMirrorBorder(border);
    this.setBorderLength(borderLength);
  }

  getPattern(isGlobal = false): RegExp {
    const pattern = `\\${this.mirrorBorder.startSymbol.split('').join('\\')}(.*?)\\${this.mirrorBorder.endSymbol
      .split('')
      .join('\\')}`;
    if (isGlobal) {
  ...
  private setMirrorBorder(border: SnippetBorder | OwnBorder): MirrorBorder {
    switch (border) {
      case SnippetBorder.BRACE:
        return new BraceBorder();
      case SnippetBorder.ROUND_BRACKET:
        return new RoundBracketBorder();
      case SnippetBorder.SQUARE_BRACKET:
        return new SquareBracketBorder();
      default:
